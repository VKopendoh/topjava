...
public class MealsUtil {
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        List<Meal> meals = generate().getAll();
...
//        System.out.println(getFilteredByClosure(meals, startTime, endTime, DEFAULT_CALORIES_PER_DAY));
    ...
    public static MealDao generate() {
        mealDao = MealInMemoryDao.get();
        mealDao.saveOrUpdate(new Meal(LocalDateTime.of(2015, Month.MAY, 30, 10, 0), "Завтрак", 500, mealDao.generateId()));
        mealDao.saveOrUpdate(new Meal(LocalDateTime.of(2015, Month.MAY, 30, 13, 0), "Обед", 1000, mealDao.generateId()));
        mealDao.saveOrUpdate(new Meal(LocalDateTime.of(2015, Month.MAY, 30, 20, 0), "Ужин", 500, mealDao.generateId()));
        mealDao.saveOrUpdate(new Meal(LocalDateTime.of(2015, Month.MAY, 31, 10, 0), "Завтрак", 1000, mealDao.generateId()));
        mealDao.saveOrUpdate(new Meal(LocalDateTime.of(2015, Month.MAY, 31, 13, 0), "Обед", 500, mealDao.generateId()));
        mealDao.saveOrUpdate(new Meal(LocalDateTime.of(2015, Month.MAY, 31, 20, 0), "Ужин", 510, mealDao.generateId()));
        return mealDao;
    }
